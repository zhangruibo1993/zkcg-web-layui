...
layui.define(["layer", 'okCookie'], function (exprots) {
   ...
   var okUtils = {
       ...
       * 服务器地址
       */
      // baseUrl: "http://127.0.0.1:8890/sale",
	  baseUrl: "https://www.luckparking.cn/sale",
      /**
       * 获取body的总宽度
       */
      ...
      ajax: function (url, type, params, load) {
         ...
         $.ajax({
            ...
            beforeSend: function (XMLHttpRequest) {
			   ...
			   XMLHttpRequest.setRequestHeader('from', 'web');
			   XMLHttpRequest.setRequestHeader('session_token', sessionStorage.getItem("session_token"));
            },
	   ...
	   * @returns {*|never|{always, promise, state, then}}
	   */
	  ajaxSync: function (url, type, params, load) {
	     var deferred = $.Deferred();
	     var loadIndex;
	     $.ajax({
	        url: okUtils.isFrontendBackendSeparate ? okUtils.baseUrl + url : url,
